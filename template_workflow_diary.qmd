...
A small helper function that we will use to check whether a package is installed and install it if necessary is provided below:

```{r}
install_if_missing <- function(pkgs, repos = "") {
  pkgs_missing <- pkgs[sapply(pkgs, \(pkg) !requireNamespace(pkg, quietly = TRUE))]
  if (length(pkgs_missing) > 0) {
    install.packages(pkgs_missing, repos = c(repos, "https://cloud.r-project.org/"))
  }
}
```
...
```{r}
install_if_missing("cmdstanr", repos = "https://mc-stan.org/r-packages/")
```
...
```{r}
install_if_missing(c("bayesplot", "loo", "posterior"))
